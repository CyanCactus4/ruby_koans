...
class Proxy
  def initialize(target_object)
    ...
    @messages = []
    @called_counts = Hash.new(0)
    # ADD MORE CODE HERE
  end
  def method_missing(method_name, *args, &block)
    if @object.respond_to?(method_name)
      @messages << method_name
      @called_counts[method_name] += 1
  ...
  # Возвращает список всех вызванных методов (в порядке вызова)
  def messages
    @messages
  end

  # Проверяет, был ли вызван метод method_name хотя бы один раз
  def called?(method_name)
    @called_counts.key?(method_name)
  end

  # Возвращает количество вызовов метода method_name
  def number_of_times_called(method_name)
    @called_counts[method_name] || 0
  end
